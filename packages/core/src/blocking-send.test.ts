...
function createStubTransport(
  statusFrames: Uint8Array[],
  kind: RuntimeTransport["kind"] = "usb"
): RuntimeTransport {
  let index = 0;
  return {
    kind,
...
describe("blocking send semantics", () => {
  it("returns sent-ambiguous for network backend without readback", async () => {
    const result = await sendBlocking({
      transport: createStubTransport([new Uint8Array([2, 0, 0, 0])], "network"),
  ...
  it("resolves completion when status indicates success", async () => {
    const result = await sendBlocking({
      transport: createStubTransport([new Uint8Array([2, 0, 0, 0])]),
      payload: new Uint8Array([0x1b, 0x40]),
      timeoutMs: 100
    });
  ...
  it("returns non-ambiguous error on explicit error frame", async () => {
    const result = await sendBlocking({
      transport: createStubTransport([new Uint8Array([255, 3, 1, 0])]),
      payload: new Uint8Array([0x1b, 0x40]),
      timeoutMs: 100
    });
  ...
  it("returns ambiguous completion on timeout", async () => {
    const result = await sendBlocking({
      transport: createStubTransport([new Uint8Array([1, 0, 0, 0])]),
      payload: new Uint8Array([0x1b, 0x40]),
      timeoutMs: 1,
      pollIntervalMs: 1
    });
